...
namespace leetcode_GasStation_cs
{
    class Program
    {
        public static int CanCompleteCircuit(int[] gas, int[] cost)
        {
            ...
            // iterate through all the possible stations : n stations  : O(n)
            for (int station_start = 0 ; station_start < station_count ; station_start++)
            {
                ...
                // starting from the start start , we visit each of the remaining stations , so worst case complexity : O(n)
        ...
        // the space complexity : 2n [ gas array and the cost array ]
